...
const DataManager = {
    ...
    addStation(station) {
        ...
        if (typeof FirebaseManager !== 'undefined' && FirebaseManager.isInitialized) {
            FirebaseManager.saveStation(station);
        }
    ...
    updateStation(lat, lng, updatedStation) {
        ...
        if (index !== -1) {
            ...
            // Update Firebase
            if (typeof FirebaseManager !== 'undefined' && FirebaseManager.isInitialized) {
                FirebaseManager.saveStation(updatedStation);
            }
    ...
    deleteStation(lat, lng) {
        ...
        if (typeof FirebaseManager !== 'undefined' && FirebaseManager.isInitialized) {
            FirebaseManager.deleteStation(lat, lng);
            FirebaseManager.deleteEtimsData(lat, lng);
        }
    ...
    updateEtimsStatus(lat, lng, status, notes = '') {
        ...
        etimsData[stationKey] = update;

        // Sync specific record to Firebase
        if (typeof FirebaseManager !== 'undefined' && FirebaseManager.isInitialized) {
            FirebaseManager.saveEtimsUpdate(lat, lng, update);
        }
